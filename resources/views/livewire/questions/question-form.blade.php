                ...
                <div class="p-6 text-gray-900">
                    <form wire:submit.prevent="save">
                        <div>
                            ...
                            <x-textarea wire:model.defer="question.question_text" id="question_text"
                                        class="block mt-1 w-full" type="text" name="question_text" required/>
                        ...
                        <div class="mt-4">
                            ...
                            @foreach($questionOptions as $index => $questionOption)
                                <div class="flex mt-2">
                                    <x-text-input type="text" wire:model.defer="questionOptions.{{ $index }}.option"
                                                  class="w-full" name="questions_options_{{ $index }}"
                                                  id="questions_options_{{ $index }}" autocomplete="off"/>

                                    <div class="flex items-center">
                                        <input type="checkbox" class="mr-1 ml-4"
                                               wire:model.defer="questionOptions.{{ $index }}.correct"> Correct
                                        <button wire:click="removeQuestionsOption({{ $index }})" type="button"
                                ...
                                <x-input-error :messages="$errors->get('questionOptions.' . $index . '.option')"
                                               class="mt-2"/>
                            @endforeach
